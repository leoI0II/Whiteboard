...
public class EraserController extends Controller {
    ...
    public EraserController(final Eraser eraser, final DrewPool itemPool, final Viewport viewport) {
        this.eraser = Objects.requireNonNull(eraser);
    ...
    protected void onMouseDragged(double x, double y) {
        ...
        for (var item : contextItems) {
            ...
            if (eraserBox.intersects(item.getBoundingBox())) {
                if (checkExactCollision(erasableItem, worldX, worldY, eraser.getContext().getRadius())) {
